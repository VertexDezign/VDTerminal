...
export function MapPanel({ env, vehicle }: MapPanelProps) {
  ...
  const [autoCenter, setAutoCenter] = useState(storage.getAsBoolean("autoCenter") ?? false);
  ...
  const centerOnPlayer = useCallback(() => {
    ...
    mapRef.current.setView(
      [mapSize.height - player.posZ * mapSize.height, player.posX * mapSize.width],
...
function MapEventBridge({
  ...
  useEffect(() => {
    ...
    map.setView(
      [mapSize.height - playerPosZ * mapSize.height, playerPosX * mapSize.width],
      map.getZoom(),
      { animate: true, duration: 0.5, easeLinearity: 1 },
    );
