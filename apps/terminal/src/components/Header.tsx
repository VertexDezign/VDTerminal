import { Calendar, Clock, Menu, Minus, Plus, Search, Thermometer, Tractor } from "lucide-react";
  ...
  return (
    <header className="bg-(--color-brand-active) text-(--color-brand-text) p-2 px-4 grid grid-cols-3 items-center shadow-md z-10">
      <div className="flex items-center gap-12">
        ...
        <div className="flex items-center gap-4 tabular-nums">
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-sm font-bold leading-tight">{getVal(vehicle?.motor?.state)}</span>
          </div>
          <div className="flex flex-col items-center">
      ...
      <div className="flex items-center justify-center gap-8 text-(--color-brand-label-text)">
        <div className="text-3xl font-black italic tracking-tighter uppercase">{brandName}</div>
      </div>
