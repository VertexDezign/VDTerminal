  ...
  try {
    ...
    if (filename.toLowerCase().endsWith(".dds")) {
      try {
        ...
        const mainMip = imageBuffer.slice(firstImage.offset, firstImage.offset + firstImage.length);
        ...
        return res.status(500).send("Failed to convert DDS image: " + err.message);
      }
    }
