...
export default function Implements({ vehicle }: { vehicle: Vehicle }) {
  ...
  const collectFillUnits = (imp: any): any[] => {
    ...
    if (imp.fillUnits?.fillUnit) {
      ...
      const fillUnitsArray = Array.isArray(fillUnitsData)
        ? fillUnitsData
        : [fillUnitsData];
  ...
  const mergeFillUnits = (units: any[]): any[] => {
    ...
    for (const unit of units) {
      ...
      if (!merged[type]) {
        ...
        merged[type].totalPercentage += parseInt(
          unit.fillLevelPercentage || "0",
        );
        merged[type].count += 1;
      }
    }
  ...
  return (
    <Panel
      ...
      headerActions={
        <button
          onClick={() =>
            setDisplayMode(displayMode === "separate" ? "merged" : "separate")
          }
